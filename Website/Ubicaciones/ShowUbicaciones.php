...
try {
    ...
    if ($rows) {
        foreach ($rows as $row) {
            ...
            echo "<td><a href='ValidarEditUbicaciones.php?id=" . $row['IDUbicacion'] . "' class='boton-editar mostrar-editar'>Editar</a></td>";
            echo "<td><a href=\"DeleteUbicacion.php?nombre=" . urlencode($row['NombreUbicacion']) . "\" onclick=\"return confirm('¿Está seguro de que desea eliminar esta ubicación?');\" class='boton-borrar mostrar-borrar'>Borrar</a></td>";
            echo "</tr>";
        }
    } else {
        echo "0 resultados";
